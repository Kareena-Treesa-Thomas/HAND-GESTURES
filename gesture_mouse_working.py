"""
GestureMouse MVP - MediaPipe 0.10+ Task API
"""
...
class GestureMouse:
    def __init__(self):
        # Create detector with model asset buffer
        # We'll use create_from_options without explicit model path
        # which will use the default built-in model
    ...
    def run(self):
        ...
        while True:
            ...
            # Detect hands
            result = self.detector.detect_for_video(mp_image, frame_count)
            frame_count += 1
            ...
            if result.hand_landmarks:
                for hand in result.hand_landmarks:
                    ...
                    ix = int(hand[8].x * self.cam_width)
                    iy = int(hand[8].y * self.cam_height)
