...
- **Scroll Control**
  ...
  - 2 fingers: Scroll up
- **Tab Switching**: 4 or 5 fingers + swipe right
- **Application Control**: Both hands showing 5 fingers each to minimize the current app
...
- **Scroll Down**: 1 finger
- **Scroll Up**: 2 fingers
- **Next Tab**: 4 or 5 fingers + swipe right
- **Minimize App**: 5 fingers on both hands
...
- `config.py`: Basic settings (camera index, text position, etc.)
...
- The `hand_landmarker.task` file is large (~15MB) and is ignored by Git (`.gitignore`). Download it separately as shown above.
